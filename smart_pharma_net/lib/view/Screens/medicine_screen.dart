...
class MedicineScreen extends StatefulWidget {
  ...
  const MedicineScreen({Key? key, required this.pharmacyId}) : super(key: key);
...
class _MedicineScreenState extends State<MedicineScreen> {
    ...
    try {
      ...
      if (_isPharmacy) {
        ...
        if (pharmacyId != null && mounted) {
          await Provider.of<MedicineViewModel>(context, listen: false)
              .loadMedicines(pharmacyId: pharmacyId);
        }
      } else if (mounted) {
        await Provider.of<MedicineViewModel>(context, listen: false).loadMedicines();
      }
    } catch (e) {
      if (mounted) {
      ...
      children: [
        Expanded(
          child: ElevatedButton(
            onPressed: () => _handleEditMedicine(medicine),
        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () => _handleDeleteMedicine(medicine),
            style: ElevatedButton.styleFrom(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: _buildDrawer(),
      floatingActionButton: _isPharmacy ? _buildAddMedicineButton() : null,
      body: Column(
        children: [
          _buildHeaderSection(),
          Expanded(
          ...
          DrawerHeader(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Welcome, ${_isAdmin ? 'Admin' : _isPharmacy ? 'Pharmacy' : 'User'}',
                  style: const TextStyle(
                    color: Colors.white,
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
  ...
  Future<void> _refreshMedicines() async {
    if (_isPharmacy && _currentPharmacyId != null) {
      await Provider.of<MedicineViewModel>(context, listen: false)
          .loadMedicines(pharmacyId: _currentPharmacyId!);
    } else {
      await Provider.of<MedicineViewModel>(context, listen: false).loadMedicines();
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (!_isAdmin && medicine.quantityToSell > 0) ...[
              const SizedBox(height: 4),
              Text(
